...
inline void ForEachOffDiagonalHessian(had::ADGraph& graph,
                                      had::VertexId outer,
                                      Consumer consumer) {
...
inline void PropagateHessianRestricted(
    ...
    const std::vector<had::VertexId>& reverse_order,
    ActivePredicate active, std::size_t* operation_count = nullptr) {
  had::g_ADGraph = &graph;
  ...
  for (had::VertexId id : reverse_order) {
    ...
    ForEachOffDiagonalHessian(graph, id, [&](had::VertexId key,
                                             had::Real value) {
      ...
      if (e1_active) {
        if (e1.to == second.to)
          graph.selfSoEdges[e1.to] +=
              had::Real(2.0) * e1.w * second.w;
        else
          AddOffDiagonalHessian(graph, e1.to, second.to,
                                e1.w * second.w);
        if (operation_count) ++*operation_count;
      }
      if (e2_active) {
        if (e2.to == second.to)
          graph.selfSoEdges[e2.to] +=
              had::Real(2.0) * e2.w * second.w;
        else
          AddOffDiagonalHessian(graph, e2.to, second.to,
                                e2.w * second.w);
        if (operation_count) ++*operation_count;
      }
    });
      ...
      if (e2_active) {
        ...
        if (e1_active) {
          if (e1.to == e2.to)
            ...
            AddOffDiagonalHessian(
                graph, e1.to, e2.to, e1.w * e2.w * diagonal);
          if (operation_count) ++*operation_count;
        }
      }
    }
    ...
    if (adjoint != had::Real(0.0)) {
      if (vertex.soW != had::Real(0.0)) {
        if (e2.to == id) {
          ...
          AddOffDiagonalHessian(
              graph, e1.to, e2.to, adjoint * vertex.soW);
          if (operation_count) ++*operation_count;
        }
      }
...
inline void PropagateRandomHessianDirectionalRestricted(
    had::ADGraph& graph, const LaplaceGraphPlan& plan) {
  had::g_ADGraph = &graph;
  ...
  for (had::VertexId id : plan.random_reverse_order()) {
      ...
      if (e2_active) {
        ...
        if (e1_active) {
          ...
          const had::Real cross_dot =
              (directional.e1Dw * e2.w +
               e1.w * directional.e2Dw) * diagonal +
    ...
    if ((adjoint != had::Real(0.0) ||
      ...
      if (e2.to == id) {
        ...
        graph.soEdgesDot[std::max(e1.to, e2.to)].Insert(
            std::min(e1.to, e2.to), create_dot);
      }
    }
...
inline bool PropagateMixedHessianFrozen(had::ADGraph& graph,
  ...
  try {
    std::call_once(topology->mixed_destination_once, [&]() {
      topology->mixed_destination_e1.assign(
          topology->nodes.size(), no_destination);
      topology->mixed_destination_e2.assign(
          topology->nodes.size(), no_destination);
      ...
      const auto set_destination =
          [&](std::vector<std::uint32_t>& destinations,
              ...
              had::VertexId key) {
            if (!plan.laplace_active(target) ||
                !keep_edge(target, key))
              return;
            if (target == key) {
            ...
            if (destination >= diagonal_destination)
              throw std::out_of_range(
                  "frozen mixed-Hessian destination is absent");
      ...
      for (had::VertexId id : plan.laplace_reverse_order()) {
        ...
        for (std::size_t slot = begin; slot < end; ++slot) {
          ...
          set_destination(topology->mixed_destination_e1, slot,
                          vertex.e1.to, key);
          if (vertex.e2.to != id)
            set_destination(topology->mixed_destination_e2, slot,
                            vertex.e2.to, key);
        }
      }
    });
  ...
  had::g_ADGraph = &graph;
  ResetHessianSweepPreserveStorage(
      graph, plan.laplace_reverse_order());
  graph.vertices[plan.objective()].w = had::Real(1.0);
    ...
    for (std::size_t slot = begin; slot < end; ++slot) {
      ...
      if (e1_active)
        accumulate_destination(
            ...
            (topology->mixed_destination_e1[slot] ==
      ...
      if (e2_active)
        accumulate_destination(
            ...
            (topology->mixed_destination_e2[slot] ==
             diagonal_destination
                 ? had::Real(2.0)
                 : had::Real(1.0)) *
                e2.w * value);
